...
#ifdef __3DS__
static void *R_3DSRealloc(void *oldptr, size_t oldsize, size_t newsize)
{
	void *p = linearAlloc(newsize);

	if (!p)
		I_Error("R_3DSRealloc: %lu bytes", (unsigned long)newsize);
	if (oldptr)
	{
		memcpy(p, oldptr, oldsize < newsize ? oldsize : newsize);
		linearFree(oldptr);
	}
	return p;
}
...
#ifdef __3DS__
      drawsegs = R_3DSRealloc(drawsegs, maxdrawsegs*sizeof (*drawsegs),
...
#ifdef __3DS__
          size_t oldmaxopenings = maxopenings;
#endif
...
#ifdef __3DS__
          openings = R_3DSRealloc(openings, oldmaxopenings * sizeof (*openings),
