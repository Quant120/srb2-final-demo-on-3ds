/* $Id: image.c,v 3.15 1998/08/21 02:41:39 brianp Exp $ */
 ...
 * and glGetTexImage?D()
 */
void gl_pack_rgba_span( const GLcontext *ctx, GLuint n, CONST GLubyte *rgba,
                        GLenum format, GLenum type, GLvoid *destination)
{
   ...
   if (!ctx->Pixel.ScaleOrBiasRGBA && !ctx->Pixel.MapColorFlag &&
      ...
      for (i=0;i<n;i++) {
         red[i]   = *(rgba + (i*4) + RCOMP) * rscale;
         green[i] = *(rgba + (i*4) + GCOMP) * gscale;
         blue[i]  = *(rgba + (i*4) + BCOMP) * bscale;
         alpha[i] = *(rgba + (i*4) + ACOMP) * ascale;
      }
