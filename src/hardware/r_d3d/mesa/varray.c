/* $Id: varray.c,v 3.6 1998/08/21 01:50:53 brianp Exp $ */
                           ...
                           const GLvoid *pointer )
{
   ...
   if (tflag) 
   {
     ...
     {
       gl_SelectTextureCoordSet( ctx, (GLenum)(GL_TEXTURE0_EXT + i) );
       gl_EnableClientState( ctx, GL_TEXTURE_COORD_ARRAY );
       gl_TexCoordPointer( ctx, tcomps, GL_FLOAT, stride, (GLubyte *) pointer + i * coffset );
     }
#if 0
     for( i = ctx->Array.TexCoordInterleaveFactor; i < MAX_TEX_COORD_SETS; i++) 
     {
       gl_SelectTextureCoordSet( ctx, (GLenum)(GL_TEXTURE0_EXT + i) );
   ...
   gl_SelectTextureCoordSet( ctx, (GLenum)(GL_TEXTURE0_EXT + coordSetSave) );
