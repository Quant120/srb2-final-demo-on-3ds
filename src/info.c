...
{
	memcpy(sprnamesbackup, sprnames, sizeof(sprnames));
	memcpy(statesbackup, states, sizeof(states));
	memcpy(mobjinfobackup, mobjinfo, sizeof(mobjinfo));
}

void P_ResetData(int flags)
{
	if (flags & 1)
		memcpy(sprnames, sprnamesbackup, sizeof(sprnamesbackup));
	
	if (flags & 2)
		memcpy(states, statesbackup, sizeof(statesbackup));
	
	if (flags & 4)
		memcpy(mobjinfo, mobjinfobackup, sizeof(mobjinfobackup));
}
