...
#ifdef __3DS__
static void M_AddonsMenu(int choice);
#endif
...
#ifdef __3DS__
	{IT_CALL    | IT_STRING, NULL, "    add-ons",   M_AddonsMenu,   108},
	{IT_CALL    | IT_STRING, NULL, "    options",   M_OptionsMenu,  116},
	{IT_CALL    | IT_STRING, NULL, "  quit  game",  M_QuitSRB2,     124},
#else
...
#define MAXADDONFILES 128
#define ADDONSPERPAGE 10
#define ADDONNAMELEN 31
...
static addonfile_t addonfiles[MAXADDONFILES];
static char addonmenutext[ADDONSPERPAGE+3][ADDONNAMELEN+1];
static menuitem_t AddonsMenu[ADDONSPERPAGE+3];
static int numaddonfiles;
static int addonpage;

static void M_DrawAddons(void);
static void M_LoadAddon(int choice);
static void M_PrevAddonPage(int choice);
static void M_NextAddonPage(int choice);
static void M_RefreshAddons(int choice);

static menu_t AddonsDef =
{
	NULL,
	NULL,
	0,
...
static boolean M_IsAddonFile(const char *name)
{
	const char *ext;
	...
	return !stricmp(ext, ".wad") || !stricmp(ext, ".soc");
}

static int M_AddonCompare(const void *a, const void *b)
{
	const addonfile_t *fa = (const addonfile_t *)a;
	const addonfile_t *fb = (const addonfile_t *)b;

	return stricmp(fa->name, fb->name);
}

static void M_BuildAddonsMenu(void)
{
	int first = addonpage*ADDONSPERPAGE;
	int i, n = 0;

	memset(AddonsMenu, 0, sizeof (AddonsMenu));
	memset(addonmenutext, 0, sizeof (addonmenutext));

	for (i = 0; i < ADDONSPERPAGE && first+i < numaddonfiles; i++, n++)
	{
		strncpy(addonmenutext[n], addonfiles[first+i].name, ADDONNAMELEN);
		addonmenutext[n][ADDONNAMELEN] = '\0';
		AddonsMenu[n].status = IT_CALL | IT_STRING;
		AddonsMenu[n].text = addonmenutext[n];
		AddonsMenu[n].itemaction = M_LoadAddon;
	}

	if (addonpage)
	{
		strcpy(addonmenutext[n], "PREVIOUS PAGE");
		AddonsMenu[n].status = IT_CALL | IT_WHITESTRING;
		AddonsMenu[n].text = addonmenutext[n];
		AddonsMenu[n++].itemaction = M_PrevAddonPage;
	}

	if (first+ADDONSPERPAGE < numaddonfiles)
	{
		strcpy(addonmenutext[n], "NEXT PAGE");
		AddonsMenu[n].status = IT_CALL | IT_WHITESTRING;
		AddonsMenu[n].text = addonmenutext[n];
		AddonsMenu[n++].itemaction = M_NextAddonPage;
	}

	strcpy(addonmenutext[n], "REFRESH");
	AddonsMenu[n].status = IT_CALL | IT_WHITESTRING;
	AddonsMenu[n].text = addonmenutext[n];
	AddonsMenu[n++].itemaction = M_RefreshAddons;

	AddonsDef.numitems = n;
	if (itemOn >= n)
		itemOn = n-1;
}

static void M_ScanAddons(void)
{
	DIR *dir;
	struct dirent *entry;
	...
	while ((entry = readdir(dir)) != NULL && numaddonfiles < MAXADDONFILES)
	{
		if (entry->d_name[0] == '.' || !M_IsAddonFile(entry->d_name))
			continue;

		strncpy(addonfiles[numaddonfiles].name, entry->d_name, MAX_WADPATH-1);
		addonfiles[numaddonfiles].name[MAX_WADPATH-1] = '\0';
		snprintf(addonfiles[numaddonfiles].path, MAX_WADPATH,
			"addons/%s", entry->d_name);
		numaddonfiles++;
	}

	closedir(dir);
	qsort(addonfiles, numaddonfiles, sizeof (addonfiles[0]), M_AddonCompare);

	if (addonpage*ADDONSPERPAGE >= numaddonfiles)
		addonpage = 0;
	M_BuildAddonsMenu();
}
...
{
	M_DrawGenericMenu();
	V_DrawCenteredString(BASEVIDWIDTH/2, 16, V_YELLOWMAP, "ADD-ONS");

	if (!numaddonfiles)
		V_DrawCenteredString(BASEVIDWIDTH/2, 32, 0, "No WAD or SOC files in addons/");
	else
		V_DrawCenteredString(BASEVIDWIDTH/2, 32, 0,
			va("Page %d/%d", addonpage+1,
			(numaddonfiles+ADDONSPERPAGE-1)/ADDONSPERPAGE));
}
...
static void M_AddonsMenu(int choice)
{
	...
	addonpage = 0;
	M_ScanAddons();
	M_SetupNextMenu(&AddonsDef);
}

static void M_LoadAddon(int choice)
{
	int filenum = addonpage*ADDONSPERPAGE+choice;
	char command[MAX_WADPATH+16];

	if (filenum < 0 || filenum >= numaddonfiles)
		return;

	snprintf(command, sizeof (command), "addfile \"%s\"\n",
		addonfiles[filenum].path);
...
{
	choice = 0;
	if ((addonpage+1)*ADDONSPERPAGE < numaddonfiles)
		addonpage++;
	M_BuildAddonsMenu();
	itemOn = 0;
}
...
#ifdef __3DS__
	strcpy(address, setupm_ip);
	swkbdInit(&swkbd, SWKBD_TYPE_NUMPAD, 2, sizeof (setupm_ip)-1);
	swkbdSetNumpadKeys(&swkbd, '.', 0);
	swkbdSetValidation(&swkbd, SWKBD_NOTEMPTY, 0, 0);
	swkbdSetHintText(&swkbd, "Server IP address");
	swkbdSetInitialText(&swkbd, address);
