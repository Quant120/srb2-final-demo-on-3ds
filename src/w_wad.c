...
#ifdef __3DS__
#define WAD_READAHEAD_SIZE 32768
#define WAD_NAMECACHE_SIZE 64

static byte *wad_readahead;
static int wad_readahead_file = -1;
static int wad_readahead_pos;
static int wad_readahead_len;
static unsigned int wad_namecache_pos;
static unsigned int wad_generation = 1;
...
static wadnamecache_t wad_namecache[WAD_NAMECACHE_SIZE];

static void W_ClearFastCache(void)
{
	wad_readahead_file = -1;
	wad_readahead_len = 0;
	wad_generation++;
	if (!wad_generation)
	{
		memset(wad_namecache, 0, sizeof(wad_namecache));
		wad_generation = 1;
	}
}
#endif
...
#ifdef __3DS__
	for (i = 0; i < WAD_NAMECACHE_SIZE; i++)
	{
		if (wad_namecache[i].generation == wad_generation
			&& !memcmp(wad_namecache[i].name, name8.s, 8))
			return wad_namecache[i].lump;
	}
#endif
	...
	for (i = numwadfiles - 1; i >= 0; i--)
	{
		...
		for (j = 0; j < wadfiles[i]->numlumps; j++, lump_p++)
		{
			if (*(int *)lump_p->name == v1 && *(int *)&lump_p->name[4] == v2)
			{
				int found = (i<<16) + j;
#ifdef __3DS__
				wad_namecache[wad_namecache_pos].generation = wad_generation;
				memcpy(wad_namecache[wad_namecache_pos].name, name8.s, 8);
				wad_namecache[wad_namecache_pos].lump = found;
				wad_namecache_pos = (wad_namecache_pos + 1) & (WAD_NAMECACHE_SIZE - 1);
#endif
				return found;
			}
		}
...
int W_ReadLumpHeader(int lump, void *dest, int size)
{
...
#ifdef __3DS__
	{
		...
		if (wad_readahead_file == wadnum
			&& l->position >= wad_readahead_pos
			&& l->position + size <= wad_readahead_pos + wad_readahead_len)
		{
			memcpy(dest, wad_readahead + (l->position - wad_readahead_pos), size);
			return size;
		}

		if (size <= WAD_READAHEAD_SIZE)
		{
			if (!wad_readahead)
				wad_readahead = malloc(WAD_READAHEAD_SIZE);

			start = l->position & ~4095;
			off = l->position - start;
			want = wadfiles[wadnum]->filesize - start;
			if (want > WAD_READAHEAD_SIZE)
				want = WAD_READAHEAD_SIZE;

			if (wad_readahead && off + size <= want
				&& lseek(handle, start, SEEK_SET) != -1)
			{
				bytesread = read(handle, wad_readahead, want);
				if (bytesread > 0)
				{
					wad_readahead_file = wadnum;
					wad_readahead_pos = start;
					wad_readahead_len = bytesread;
					if (off + size <= bytesread)
					{
						memcpy(dest, wad_readahead + off, size);
						return size;
					}
				}
			}
